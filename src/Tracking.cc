...
Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase *pKFDB, cvsl::Camera *pCamera, cvsl::Parameter &params) :
    mState(NO_IMAGES_YET), mTrackedFr(0), mbStep(false),
    mbOnlyTracking(false), mbMapUpdated(false), mbVO(false), mpORBVocabulary(pVoc), mpKeyFrameDB(pKFDB),
    mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys), mpViewer(NULL),
    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr)
{
    ...
    // Load ORB parameters
    bool b_parse_orb = ParseORBParamFile(params);
    if(!b_parse_orb)
    {
        std::cout << "*Error with the ORB parameters in the config file*" << std::endl;
    }
...
#ifdef CVSL_ENABLE_SYSTEM_ORBSLAM3
bool Tracking::ParseCamParamFile(cvsl::Camera *pCamera)
{
    mDistCoef = cv::Mat::zeros(4, 1, CV_32F);
    cout << endl << "Camera Parameters: " << endl;
    bool b_miss_params = false;
...
bool Tracking::ParseORBParamFile(cvsl::Parameter &param)
{
    int nFeatures, nLevels, fIniThFAST, fMinThFAST;
    float fScaleFactor;

    param.get<int>("ext.orb.features",  nFeatures);
    param.get<int>("ext.orb.thFastMin", fMinThFAST);
    param.get<int>("ext.orb.thFastIni", fIniThFAST);
    param.get<int>("ext.orb.levels",    nLevels);
    param.get<float>("ext.orb.factor",  fScaleFactor);
